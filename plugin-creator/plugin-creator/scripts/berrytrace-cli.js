  ...
  try {
    ...
    if (view) {
      ...
      const globalReactPlugin = {
        ...
        setup(buildContext) {
          buildContext.onResolve({ filter: /.*/ }, (args) => {
            const matches = ["react", "react-dom", "react-dom/client", "zustand", "lucide-react", "xterm", "xterm-addon-fit", "react-tooltip"];
...
async function main() {
  ...
  switch (command) {
    ...
    case "build": {
      ...
      const success = await performSdkBuild(pluginPath);
      if (!success) {
        process.exit(1);
      }
      break;
    ...
    default: {
      ...
      console.log("  berrytrace-cli build [path]          - \u4F7F\u7528 SDK \u9ED8\u8BA4\u914D\u7F6E\u81EA\u52A8\u6784\u5EFA/\u7F16\u8BD1\u63D2\u4EF6\u6E90\u7801");
