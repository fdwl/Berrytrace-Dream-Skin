...
## ⚡ 快速路径：SDK 内置构建与开发工作流（推荐）

**绝大多数插件不需要写 `bundle.mjs`**。`berrytrace-cli` 内置了完整的 esbuild 构建、打包与热更新流程：

```bash
# 1. 启动热监视开发工作流 (自动监听变动 ➔ 打包 ➔ 安装至 ~/.berrytrace/plugins/ ➔ 宿主实时刷新)
berrytrace-cli dev [path]

# 2. 一键编译打包 (.btp) 并更新安装到本地宿主环境刷新
berrytrace-cli install [path]
# 或：
berrytrace-cli pack [path] --install

# 3. 未打包源码原地构建，并在构建成功后自动通知宿主热重载
...
| `framer-motion` / `motion` | `window.FramerMotion` |
| `xterm` / `xterm-addon-fit` | `window.XTerm` |
| `react-tooltip` | `window.ReactTooltip` |
...
**SDK 内置构建的触发逻辑**：
- `berrytrace-cli dev` / `install` → 自动执行零配置 esbuild 编译（或跑插件 `npm run build`）➔ 签名打成 `.btp` ➔ 解压覆盖至 `~/.berrytrace/plugins/<id>` ➔ 请求 LocalServer `/reload-plugin`。
...
> - 其他 95% 的情况：直接用 `berrytrace-cli dev` 或 `berrytrace-cli build`，不写 bundle.mjs
...
| `window.FramerMotion` | Framer Motion 导出 | `framer-motion` / `motion` |
| `window.XTerm` | xterm 终端渲染与 FitAddon 插件 | `xterm` / `xterm-addon-fit` |
| `window.ReactTooltip` | ReactTooltip 元素浮框组件 | `react-tooltip` |
...
- **view 构建（browser）**：用 esbuild + `globalShimPlugin`，将 `react`/`react-dom`/`zustand`/`lucide-react`/`framer-motion`/`xterm`/`react-tooltip` 替换为 `window.*` 全局变量读取；`berrytrace-plugin-sdk` 可直接 bundle（SDK 是 symlink，不含 native 代码）。
