...
### 1. 统一构建与自动重载指令 (CLI 零配置)

在插件根目录或指定插件路径下直接运行：

```bash
# 使用 SDK 自动构建插件，并在构建成功后自动通知宿主 LocalServer 热重载
...
> **工作原理**：
> 1. `berrytrace-cli build` 使用内置的零配置 esbuild 引擎自动将 `src/` 入口编译为 `dist/`。
> 2. 构建成功后，CLI 会读取 `~/.berrytrace/local_server.json` 中的访问 Token。
> 3. 向宿主 LocalServer (`http://127.0.0.1:31828`) 发送 `/reload-plugin?id=<pluginId>` 请求；若插件尚未加载过，则自动发起 `/load-unpacked-plugin?path=<absolutePath>` 请求，实现开箱即用的插件热重载。
...
```
改代码 → npx berrytrace-plugin-sdk build --reload → 宿主实时应用变更
```
